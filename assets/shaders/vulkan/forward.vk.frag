...
layout(set = 0, binding = 2) uniform ShadowSettingsUbo {
  vec4 params0;   // x = cascadeBlendRatio, y = pcfRadius, z = pcfSamples, w =
                  // cascadeCount.
  ivec4 params1;  // x = debugSingleCascade, y = debugCascades, z =
                  // disableBlending, w = unused.
}
shadowSettings;
...
struct GpuShadowData {
  mat4 viewProj;
  vec4 cascadeData;  // x = splitNear, y = splitFar, z = layerIndex.
  vec4 biasData;     // x = biasConstant, y = biasSlope.
};

layout(std430, set = 2, binding = 8) readonly buffer InLightsSsbo {
  GpuLight inLights[];
};

layout(std430, set = 2, binding = 9) readonly buffer InShadowsSsbo {
...
// Poisson disk offsets for Poisson-style PCF.
...
// Tiny hash used to rotate the Poisson kernel per world-space location.
float hash(vec2 p) {
...
// Stable Poisson kernel rotation in world space.
...
// 3. Sample a single compare tap or a rotated Poisson PCF kernel.
...
float computeDirectionalShadow(GpuLight light, vec3 worldPos, vec3 shadowNormal,
  ...
  // Compute blend factor near far edge.
