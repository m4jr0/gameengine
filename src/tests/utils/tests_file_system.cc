...
auto tmp_dir{comet::utils::filesystem::Append(current_dir, test_dir)};
...
std::string FormatAbsolutePath(std::string_view absolute_path,
  ...
  uindex index_to_cut{
      utils::string::GetLastNthPos(absolute_path, to_search, count)};
...
TEST_CASE("File system management", "[comet::utils::filesystem]") {
  SECTION("Create operations.") {
    REQUIRE(!comet::utils::filesystem::CreateFile(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test")));

    REQUIRE(comet::utils::filesystem::CreateFile(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test"),
        true));

    comet::utils::filesystem::GetChecksum(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test"));

    REQUIRE(
        !comet::utils::filesystem::CreateFile(comet::utils::filesystem::Append(
            comet::comettests::tmp_dir, "/test2/")));

    REQUIRE(comet::utils::filesystem::CreateDirectory(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir,
                                         "/test3")));
  ...
  SECTION("Read & write operations.") {
    ...
    REQUIRE(comet::utils::filesystem::WriteStrToFile(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test7"),
        test_write));
  ...
  SECTION("Move operations.") {
    ...
    REQUIRE(comet::utils::filesystem::Move(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test8"),
        comet::utils::filesystem::Append(comet::comettests::tmp_dir,
                                         "/test8")));

    // Checking with a folder.
    REQUIRE(comet::utils::filesystem::Move(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test4"),
        comet::utils::filesystem::Append(comet::comettests::tmp_dir,
                                         "/test9")));

    // Checking with a folder on itself.
    REQUIRE(comet::utils::filesystem::Move(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test9"),
        comet::utils::filesystem::Append(comet::comettests::tmp_dir,
                                         "/test9")));
  ...
  SECTION("List operations.") {
    ...
    dir_to_test.push_back(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test3"));
    dir_to_test.push_back(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test5"));
    dir_to_test.push_back(
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test9"));
    ...
    const auto does_not_exist{comet::utils::filesystem::Append(
  ...
  SECTION("String operations.") {
    ...
    const auto dir_path{
        comet::utils::filesystem::Append(comet::comettests::tmp_dir, "/test3")};
    const auto does_not_exist_path{comet::utils::filesystem::Append(
        comet::comettests::tmp_dir, "/DOESNOTEXIST")};
    ...
    REQUIRE(comet::comettests::FormatAbsolutePath(tmp_file_path, "/", 2) ==
            "/" + comet::utils::filesystem::Append(comet::comettests::test_dir,
                                                   "/test8"));
    ...
    REQUIRE(comet::utils::filesystem::GetNameView(file_path) == "test8");
    REQUIRE(comet::utils::filesystem::GetNameView(dir_path) == "test3");
    REQUIRE(comet::utils::filesystem::GetNameView(does_not_exist_path) ==
            "DOESNOTEXIST");
    ...
    REQUIRE(comet::utils::filesystem::ReplaceExtensionToCopy(
                "fla", "/test/test1.txt.MP3") == "/test/test1.txt.fla");
