...
#ifndef COMET_COMET_MATH_MATH_SCALAR_H_
#define COMET_COMET_MATH_MATH_SCALAR_H_
...
namespace math {
template <typename T>
constexpr T SearchSqrt(T x, T low, T high) {
  if (low == high) {
  ...
  if (x / mid < mid) {
    return SearchSqrt<T>(x, low, mid - 1);
  }

  // Result is higher than low.
  return SearchSqrt(x, mid, high);
}

template <typename T>
constexpr T ISqrt(T x) {
  return SearchSqrt<T>(x, 0, x / 2 + 1);
}
...
constexpr f32 FastInvSqrt(f32 x) {
  ...
  conv.f *= 1.5f - (x * .5f * conv.f * conv.f);  // Newton step.
  return conv.f;
}
...
#endif  // COMET_COMET_MATH_MATH_SCALAR_H_
