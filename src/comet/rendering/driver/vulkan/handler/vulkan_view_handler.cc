...
void ViewHandler::OnInitialize() {
...

  {
    ...
    shader_descr.render_pass_handle = RenderPassHandle::Invalid();
...

  {
    ...
    shader_descr.shader_resource_id =
        resource::GenerateResourceIdFromPath<resource::ShaderResource>(
            COMET_TCHAR("shaders/vulkan/shadow_cull_shader.vk.cshader"));
    shader_descr.render_pass_handle = RenderPassHandle::Invalid();
    shader_descr.bind_type = PipelineBindType::Compute;
...

  {
    ...
    shader_descr.shader_resource_id =
        resource::GenerateResourceIdFromPath<resource::ShaderResource>(
            COMET_TCHAR("shaders/vulkan/rendering_debug_generate.vk.cshader"));
    shader_descr.bind_type = PipelineBindType::Compute;
    shader_descr.render_pass_handle = {};
...
void ViewHandler::PrepareFrameGpuData(frame::FramePacket* packet) {
  ...
  const auto frame{context_->GetFrameCount()};
...

  {
    ...
    camera_handler_->PopulateCameraReadBarrier(frame_index, barriers);
...
void ViewHandler::RunForwardCull(frame::FramePacket* packet) {
  ...
  if (debug_gpu_data.ssbo_debug_data_handle != VK_NULL_HANDLE) {
    AddBufferBinding(buffer_bindings,
                     shader_handler_->GetBindingIndex(
                         forward_cull_shader_, shaderconsts::kPassSet,
  ...
  if (debug_gpu_data.ssbo_debug_aabbs_handle != VK_NULL_HANDLE) {
    AddBufferBinding(buffer_bindings,
                     shader_handler_->GetBindingIndex(
                         forward_cull_shader_, shaderconsts::kPassSet,
  ...
  {
    auto& barriers{*COMET_FRAME_ARRAY_WITH_CAPACITY(VkBufferMemoryBarrier, 1)};
    debug_handler_->PopulateCullDebugReadBarriers(frame_index, barriers);
...
void ViewHandler::UpdateShadowCullShaderPassData() {
  ...
  AddBufferBinding(buffer_bindings,
                   shader_handler_->GetBindingIndex(
                       shadow_cull_shader_, shaderconsts::kPassSet,
                       sharedshaderconsts::kIndirectProxiesBinding),
                   shadow_indirect.handle, shadow_indirect.size);
...
void ViewHandler::RunShadowCull(const ShadowRenderJob& job,
  ...
  COMET_ASSERT(shadow_proxy_ids.handle != VK_NULL_HANDLE,
               "ViewHandler::RunShadowCull",
               "shadow proxy ids buffer handle is invalid");
  COMET_ASSERT(shadow_indirect.handle != VK_NULL_HANDLE,
               "ViewHandler::RunShadowCull",
  ...
  {
    auto& barriers{*COMET_FRAME_ARRAY_WITH_CAPACITY(VkBufferMemoryBarrier, 2)};
    render_proxy_handler_->PopulateShadowCullBarriers(frame_index, barriers);
...
void ViewHandler::RunCullingDebugGeneration() {
  ...
  const auto camera_frustum_count =
      debug_handler_->GetDebugCameraFrustumCount();
  ...
  AddBufferBinding(buffer_bindings,
                   ...
                   debug_handler_->GetDebugCascadeFrustumCornersBuffer().handle,
                   debug_handler_->GetDebugCascadeFrustumCornersBuffer().size);

  AddBufferBinding(buffer_bindings,
                   shader_handler_->GetBindingIndex(
                       rendering_debug_generate_shader_, shaderconsts::kPassSet,
                       sharedshaderconsts::kDebugLightFrustumsBinding),
                   frustums_buffer.handle, frustums_buffer.size);

  AddBufferBinding(buffer_bindings,
                   shader_handler_->GetBindingIndex(
                       rendering_debug_generate_shader_, shaderconsts::kPassSet,
                       sharedshaderconsts::kDebugCameraFrustumsBinding),
                   debug_handler_->GetDebugCameraFrustumBuffer().handle,
                   debug_handler_->GetDebugCameraFrustumBuffer().size);
