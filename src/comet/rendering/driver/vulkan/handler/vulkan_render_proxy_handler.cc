...
void RenderProxyHandler::UploadRenderProxyLocalData(
  ...
  if (static_cast<f32>(sparse_upload_count) >
      static_cast<f32>(record_slot_count) * kReuploadAllLocalDataThreshold_) {
    UploadAllRenderProxyLocalData(frame_index);
  } else {
...
void RenderProxyHandler::ReallocateRenderProxyDrawBuffers(
  ...
  RecreateBuffer(indirect, allocator_handle, ssbo_indirect_proxies_buffer_size,
                 VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                     VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                     VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT,
                 VMA_MEMORY_USAGE_GPU_ONLY, 0, 0, VK_SHARING_MODE_EXCLUSIVE,
  ...
  RecreateBuffer(staging_shadow_indirect, allocator_handle,
  ...
  RecreateBuffer(instances, allocator_handle, ssbo_proxy_instances_buffer_size,
                 VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
                     VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                     VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                 VMA_MEMORY_USAGE_GPU_ONLY, 0, 0, VK_SHARING_MODE_EXCLUSIVE,
...
void RenderProxyHandler::UploadRenderDrawData(FrameInFlightIndex frame_index) {
  ...
  vkCmdCopyBuffer(upload_command_buffer_handle, staging_shadow_indirect.handle,
  ...
  AddUploadReleaseBarrier(frame_index, shadow_indirect);
  ...
  AddBufferMemoryBarrier(
      shadow_indirect, post_update_barriers_, VK_ACCESS_NONE,
      VK_ACCESS_INDIRECT_COMMAND_READ_BIT | VK_ACCESS_SHADER_READ_BIT,
      upload_queue.family_index, graphics_queue.family_index);
...
void RenderProxyHandler::PopulateRenderIndirectProxy(
  ...
  const auto* mesh_proxy{mesh_handler_->TryGet(record.proxy.mesh_handle)};
...
void RenderProxyHandler::AssertRuntimeInvariants(
  ...
  COMET_ASSERT(
      ...
      "RenderProxyHandler::AssertRuntimeInvariants",
      "shadow indirect proxies buffer handle is null", "frame_index",
      frame_index);
