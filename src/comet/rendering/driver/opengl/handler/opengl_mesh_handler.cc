...
internal::UpdateContext MeshHandler::PrepareUpdate(
  ...
  update_context.staging_buffer = COMET_FRAME_ALLOC(total_size);
...
void MeshHandler::AddMeshProxies(const frame::AddedGeometries* geometries,
  ...
  for (const auto& geometry : *geometries) {
    auto vertex_size{static_cast<GLsizeiptr>(geometry.vertices->GetSize() *
                                             sizeof(geometry::Vertex))};
    auto index_size{static_cast<GLsizeiptr>(geometry.indices->GetSize() *
...
void MeshHandler::UpdateMeshProxies(const frame::DirtyMeshes* meshes,
  ...
  for (const auto& mesh : *meshes) {
    ...
    auto new_vertex_size{static_cast<GLsizeiptr>(mesh.vertices->GetSize() *
                                                 sizeof(geometry::Vertex))};
    auto new_index_size{static_cast<GLsizeiptr>(mesh.indices->GetSize() *
                                                sizeof(geometry::Index))};
  ...
  if (!update_context.index_copy_regions.IsEmpty()) {
    ...
    for (const auto& region : update_context.index_copy_regions) {
      memory::CopyMemory(
          ...
          memory + region.dst_offset, region.size);
    }
...
void MeshHandler::ResizeVertexBuffer(GLsizeiptr new_size) {
...
void MeshHandler::ResizeIndexBuffer(GLsizeiptr new_size) {
