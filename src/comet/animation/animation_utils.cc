...
AnimationPose PopulatePoseFromSamples(const CompressedAnimationClip& clip,
  ...
  COMET_ASSERT(a_sample.joint_poses.GetSize() == b_sample.joint_poses.GetSize(),
               ...
               COMET_STRING_ID_LABEL(clip.id.GetValue()), " at frames ",
               frame_a, "/", frame_b, ": ", a_sample.joint_poses.GetSize(),
...
void PopulateGlobalPose(const geometry::Skeleton& skeleton,
  ...
  for (usize i{0}; i < joint_count; ++i) {
    ...
    const auto translation{math::ToTranslateMatrix(local_pose.translation)};
    const auto rotation{math::ToRotationMatrix(local_pose.rotation)};
    const auto scale{math::ToScaleMatrix(local_pose.scale)};
