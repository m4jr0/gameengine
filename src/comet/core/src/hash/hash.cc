...
HashValue GenerateHash(const schar* value, usize length) {
  ...
  return EndHash(ContinueHash(BeginHash(), value, length));
}
...
HashValue BeginHash() { return kFnvOffsetBasis64; }

HashValue ContinueHash(HashValue hash, const void* data, usize length) {
...
HashValue ContinueHash(HashValue hash, const schar* value) {
  if (value == nullptr) {
    return hash;
  }

  while (*value != '\0') {
    hash ^= static_cast<HashValue>(static_cast<u8>(*value));
    hash *= kFnvPrime64;
    ++value;
  }
...
HashValue ContinueHash(HashValue hash, CTStringView value) {
  return ContinueHash(hash, value.GetCTStr(), value.GetLength());
}

HashValue EndHash(HashValue hash) { return hash; }
}  // namespace comet
