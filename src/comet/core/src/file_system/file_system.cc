...
struct FileSystemState {
  FileSystemAllocatorGetter get_scratch_allocator{nullptr};
};

FileSystemState g_file_system_state{};

memory::Allocator& GetScratchAllocator() {
  COMET_ASSERT(g_file_system_state.get_scratch_allocator != nullptr,
               "file_system::GetScratchAllocator",
               "scratch allocator getter is null");

  auto* allocator{g_file_system_state.get_scratch_allocator()};
  COMET_ASSERT(allocator != nullptr, "file_system::GetScratchAllocator",
               "scratch allocator is null");

  return *allocator;
}

tchar* AllocateTmpPath(usize len) {
  constexpr usize kSmallTmpPathRingCount{8};

  if (len + 1 <= kMaxPathLength) {
    thread_local tchar buffers[kSmallTmpPathRingCount][kMaxPathLength]{};
    thread_local usize index{0};

    auto* buffer{buffers[index]};
    index = (index + 1) % kSmallTmpPathRingCount;
    return buffer;
  }

  COMET_ASSERT(g_file_system_state.get_scratch_allocator != nullptr,
               "file_system::AllocateTmpPath",
               "scratch allocator is required for large temporary path",
               "required_len", len + 1);

  return GetScratchAllocator().AllocateMany<tchar>(len + 1);
}

const tchar* internal::GetTmpCopyWithNormalizedSlashes(CTStringView str) {
#ifdef COMET_WINDOWS
  auto* tmp{AllocateTmpPath(str.GetLength())};

  for (usize i{0}; i < str.GetLength(); ++i) {
    auto c{str[i]};
    tmp[i] = c == COMET_TCHAR('\\') ? COMET_TCHAR('/') : c;
  }
...
void InitializeFileSystem(const FileSystemContext& context) {
  internal::g_file_system_state.get_scratch_allocator =
      context.get_scratch_allocator;
}

void ShutdownFileSystem() { internal::g_file_system_state = {}; }
...
bool Remove(CTStringView path, bool is_recursive) {
  ...
  auto* full_path{internal::AllocateTmpPath(full_path_capacity)};
  ...
  do {
    ...
    auto* sub_path{internal::AllocateTmpPath(sub_path_len)};
...
TString GetNormalizedPath(CTStringView path) {
  ...
  auto* normalized{internal::AllocateTmpPath(path.GetLength())};
  ...
  for (usize i{start_anchor}; i < path.GetLength(); ++i) {
    const auto c{path[i]};
...
TString GetAbsolutePath(CTStringView relative_path) {
  ...
  if (!is_ok) {
    ...
    if (last_slash != kInvalidIndex) {
      auto* tmp{internal::AllocateTmpPath(len)};
...
TString GetRelativePath(CTStringView to, CTStringView from) {
  ...
  auto* relative_path{internal::AllocateTmpPath(relative_path_len)};
  ...
  const auto to_copy_len{static_cast<usize>(to_end - to_cursor)};
...
void MakeNative([[maybe_unused]] tchar* str, [[maybe_unused]] usize len) {
#ifdef COMET_MSVC
  for (usize i{0}; i < len; ++i) {
    if (str[i] == COMET_TCHAR('/')) {
      str[i] = COMET_TCHAR('\\');
...
const tchar* GetTmpTChar(const wchar* str, [[maybe_unused]] usize len) {
#ifdef COMET_WIDE_TCHAR
  return str;
#else
  auto* tmp{internal::AllocateTmpPath(len)};
...
usize GetSize(CTStringView path) {
  ...
  if (stat(path.GetCTStr(), &status) == 0) {
    return static_cast<usize>(status.st_size);
  }
