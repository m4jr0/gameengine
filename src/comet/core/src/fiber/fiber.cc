...
namespace fiber {
namespace internal {
static memory::Allocator* fiber_stack_allocator{nullptr};

memory::Allocator& GetFiberStackAllocator() {
  COMET_ASSERT(fiber_stack_allocator != nullptr,
               "fiber::internal::GetFiberStackAllocator",
               "fiber stack allocator is null");
  return *fiber_stack_allocator;
}
}  // namespace internal
...
void Fiber::Initialize() {
  ...
  stack_ =
      static_cast<Stack>(internal::GetFiberStackAllocator().AllocateAligned(
          GetAllocatedStackSize(stack_capacity_), memory::kStackAlignment));
...
void Fiber::Destroy() {
  if (stack_ != nullptr) {
    internal::GetFiberStackAllocator().Deallocate(stack_);
    stack_ = nullptr;
  }
}
...
void AttachFiberStackAllocator(memory::Allocator* allocator) {
  COMET_ASSERT(allocator != nullptr, "fiber::AttachFiberStackAllocator",
               "allocator is null");
  COMET_ASSERT(internal::fiber_stack_allocator == nullptr,
               "fiber::AttachFiberStackAllocator",
               "fiber stack allocator is already attached");
  internal::fiber_stack_allocator = allocator;
}

void DetachFiberStackAllocator() {
  COMET_ASSERT(internal::fiber_stack_allocator != nullptr,
               "fiber::DetachFiberStackAllocator",
               "fiber stack allocator is not attached");
  internal::fiber_stack_allocator = nullptr;
}

bool IsFiberStackAllocatorAttached() {
  return internal::fiber_stack_allocator != nullptr;
}
// namespace internal
}  // namespace fiber
}  // namespace comet