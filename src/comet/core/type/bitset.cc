...
void Bitset::Set(usize index) {
  ...
  words_[index / kWorkBitCount_] |= static_cast<Word>(1)
                                    << (index % kWorkBitCount_);
}
...
bool Bitset::Test(usize index) const {
  ...
  return (words_[index / kWorkBitCount_] &
          (static_cast<Word>(1) << (index % kWorkBitCount_))) != 0;
}
...
void Bitset::Resize(usize new_bit_count) {
  ...
  if (words_ != nullptr) {
    const auto full_old_words{old_bit_count / kWorkBitCount_};
    memory::CopyMemory(new_words, words_, full_old_words * sizeof(Word));

    const auto remaining_bits{old_bit_count % kWorkBitCount_};
