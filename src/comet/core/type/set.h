  ...
  class IteratorImpl {
   public:
    ...
    IteratorImpl(BucketIterator bucket_it, BucketIterator bucket_end,
        ...
        : bucket_it_{bucket_it}, bucket_end_{bucket_end}, elem_it_{elem_it} {
      SkipEmptyBuckets();
    }

    reference operator*() const noexcept { return *elem_it_; }

    pointer operator->() const noexcept { return elem_it_; }

    IteratorImpl& operator++() {
      ++elem_it_;

      if (elem_it_ == bucket_it_->end()) {
        ++bucket_it_;
        SkipEmptyBuckets();
      }
    ...
    bool operator==(const IteratorImpl& other) const noexcept {
      return bucket_it_ == other.bucket_it_ &&
             (bucket_it_ == bucket_end_ || elem_it_ == other.elem_it_);
    }
