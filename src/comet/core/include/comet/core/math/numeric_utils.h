...
#ifndef COMET_CORE_MATH_NUMERIC_UTILS_H_
#define COMET_CORE_MATH_NUMERIC_UTILS_H_
...
constexpr T AvoidZero(T v, T eps = kEpsilonV<T>) noexcept {
  return IsAlmostZero(v, eps) ? (v < T{0} ? -eps : eps) : v;
}
}  // namespace math
}  // namespace comet

#endif  // COMET_CORE_MATH_NUMERIC_UTILS_H_
