...
// Header. /////////////////////////////////////////////////////////////////////
#include "comet/render/light_manager.h"
////////////////////////////////////////////////////////////////////////////////
...
void LightManager::Update(frame::FramePacket* packet) {
  ...
  COMET_ASSERT(packet != nullptr, "LightManager::Update",
...
void LightManager::Destroy(LightHandle handle) {
  if (IsShutdownPending()) {
    const auto index{static_cast<usize>(handle.GetIndex())};
    COMET_ASSERT(index < lights_.GetSize(), "LightManager::Destroy",
                 "light handle index is out of bounds", "light_handle", handle,
                 "index", index, "light_count", lights_.GetSize());
  ...
  COMET_ASSERT(destroyed_light_handles_ != nullptr, "LightManager::Destroy",
               "destroyed light handle set is null");
  COMET_ASSERT(new_light_handles_ != nullptr, "LightManager::Destroy",
               "new light handle set is null");
...
void LightManager::EmitFramePacketChanges(frame::FramePacket* packet) {
  ...
  COMET_ASSERT(packet != nullptr, "LightManager::EmitFramePacketChanges",
               "frame packet is null");
...
Light* LightManager::Get(LightHandle handle) {
  COMET_ASSERT(light_pool_.IsAlive(handle), "LightManager::Get",
               "light is not alive", "light_handle", handle);
  const auto index{static_cast<usize>(handle.GetIndex())};
  COMET_ASSERT(index < lights_.GetSize(), "LightManager::Get",
               "light handle index is out of bounds", "light_handle", handle,
               "index", index, "light_count", lights_.GetSize());
