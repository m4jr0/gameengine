...
constexpr bool kIsMeshStressEnabled{true};
...
struct AsyncModelGroupFence {
  ...
  bool Poll() const noexcept {
    return group != nullptr &&
           group->completed_count.load(std::memory_order_acquire) >=
               group->job_count;
  }
};
...
static void StartSceneModelJobs() {
  ...
  scene_load_group_tmp.state = AsyncGroupState::Loading;
...
static void LoadTmp() {
  COMET_PROFILE("SceneManager::LoadTmp");
...
static void StartRenderProxySpawnTmp() {
  ...
  if (render_proxy_group_tmp.state == AsyncGroupState::Loading ||
  ...
  render_proxy_group_tmp.state = AsyncGroupState::Loading;
...
static void StressComponentAddRemove() {
  ...
  const auto baseline_count{entity_manager.GetEntityCount()};
