...
#ifndef KOMA_CORE_RENDERING_SHADER_SHADER_LOADER_HPP_
#define KOMA_CORE_RENDERING_SHADER_SHADER_LOADER_HPP_
...
namespace koma {
  ...
  GLuint CreateShaderProgram(Targs... shader_ids) {
    ...
    if (info_log_len > 0) {
      ...
      glGetProgramInfoLog(
        program_id, info_log_len,
        nullptr,
        &error_message[0]
      );

      logger->Error(
        "Error while creating shader programs"
      );
...
#endif  // KOMA_CORE_RENDERING_SHADER_SHADER_LOADER_HPP_
